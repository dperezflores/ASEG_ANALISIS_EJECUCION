...
class GeminiProvider:
    ...
    def _llamar(self, contenidos: list, esquema: Type[BaseModel], max_reintentos: int = 3):
    ...
    def analizar_pdf(self, archivo_pdf, prompt: str, esquema: Type[BaseModel]) -> ResultadoExtraccion:
        try:
            ...
            datos = contenido_json if isinstance(contenido_json, list) else [contenido_json]
            usage = getattr(response, "usage_metadata", None)
            consumo = usage.model_dump(mode="json") if hasattr(usage, "model_dump") else {}
            ...
            return ResultadoExtraccion(
                ...
                metadatos={"proveedor": "Gemini", "modelo": self.model_name},
            )
        except Exception as exc:
            return ResultadoExtraccion(
                estado="ERROR",
